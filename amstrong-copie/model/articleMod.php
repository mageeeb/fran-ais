...
function postAdminInsert(PDO $db, int $idUser, string $postTitle, string $postMin, string $postMax, string $postSound, string $postImg1, string $postImg2, string $postImg3, array $idCateg = []): bool
{
   ...
   /* $preparePost->bindValue(":url_image_1", $postImg1, PDO::PARAM_STR);
    $preparePost->bindValue(":url_image_2", $postImg2, PDO::PARAM_STR);
    $preparePost->bindValue(":url_image_3", $postImg3, PDO::PARAM_STR);*/
    ...
    if (!empty($postImg1)) {
        $postInsert= $db->prepare("INSERT INTO `image` VALUES :article_id_article");
        // $valeur par défaut (sera remplacée en cas de validité du tableau)
        $categId = 100;

        // attribution des valeurs par référence, $value est donc une valeur par défaut qui ne sera pas utilisée
        //$postInsert->bindParam("category_id_category", $categId, PDO::PARAM_INT);
        $postInsert->bindParam("article_id_article", $postLastInsertId, PDO::PARAM_INT);
    }
